...
<footer id="footer" class="footer">
  ...
  <div class="footer-top">
    <div class="container">
      <div class="row gy-4">
        <div class="col-lg-5 footer-info card shadow-none">
          <a href="<?= base_url('home') ?>" class="logo d-flex align-items-center">
            <img src="<?= base_url('assets/img/company/' . $image) ?>" alt="">
            <span><?= esc($setting->nama_perusahaan) ?></span>
          </a>
          <p><?= esc($setting->profile) ?></p>
        ...
        <div class="col-lg-2 col-6 footer-links">
          ...
          <ul>
            <?php foreach ($layanan as $value): ?>
              <li><i class="bi bi-chevron-right"></i> <a href="#"><?= esc($value->judul_layanan) ?></a></li>
            <?php endforeach ?>
          </ul>
        </div>
        ...
        <div class="col-lg-3 footer-contact text-md-start">
          <h4>Contact Us</h4>
          <p>
            <?= esc($setting->alamat) ?><br>
            <strong>Phone:</strong> <?= esc($setting->no_telepon) ?><br>
            <strong>Email:</strong> <?= esc($setting->email) ?><br>
          </p>
        </div>
      </div>
    </div>
  ...
  <div class="container">
    <div class="copyright">
      &copy; <?= date('Y') ?> <strong><span><?= esc($setting->nama_perusahaan) ?></span></strong>. All Rights Reserved
    </div>
  </div>
</footer>
